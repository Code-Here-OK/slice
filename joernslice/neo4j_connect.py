...
def get_nodes_by_lines(lines, directory, type):
    ...
    if not os.path.exists('/root/joernSlice/data/graph/' + type + '/' + directory):
        os.mkdir('/root/joernSlice/data/graph/' + type + '/' + directory)
    with open('/root/joernSlice/data/graph/' + type + '/' + directory + '/nodes.json', 'w') as f:
...
def get_edges_by_nodes_id(ids, directory, type):
    ...
    if not os.path.exists('/root/joernSlice/data/graph/' + type + '/' + directory):
        os.mkdir('/root/joernSlice/data/graph/' + type + '/' + directory)
    with open('/root/joernSlice/data/graph/' + type + '/' + directory + '/edges.json', 'w') as f:
        f.write(json.dumps(result, indent=4))
