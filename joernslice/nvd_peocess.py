    ...
    for cve in tqdm(os.listdir(code_path)):
        ...
        for file in os.listdir(os.path.join(code_path, cve)):
            if file.endswith('.c') or file.endswith('.cpp'):
                ...
                joern_parse_path = f"/joern-cli/joern-parse"
                cpg_output_path = f"/data/nvd/cpg_bin/{cve_id}"
...
def neo4jcsv_export():
    ...
    for directory in tqdm(os.listdir(bin_path), desc='generating neo4jcsv'):
        ...
        csv_output_path = f"/data/nvd/cpg_csv/{directory}/"
...
def get_nodes_by_lines(lines, directory, type):
    ...
    if not os.path.exists('/data/nvd/graph/' + directory):
        os.mkdir('/data/nvd/graph/' + directory)
    with open('/data/nvd/graph/' + directory + '/nodes.json', 'w') as f:
...
def get_edges_by_nodes_id(ids, directory, type):
    ...
    if not os.path.exists('/data/nvd/graph/' + directory):
        os.mkdir('/data/nvd/graph/' + directory)
    with open('/data/nvd/graph/' + directory + '/edges.json', 'w') as f:
        f.write(json.dumps(result, indent=4))      
def clear_all_data():
    graph = connect()
    query = """
...
if __name__ == "__main__":
    ...
    #     else:
    #         with open('/data/nvd/error.txt', 'a+') as f:
    #             f.write(directory + '\n')
        ...
        #     neo4jhandler.neo4j_stop()
        # else:
        #     with open('/data/exception/slice_good_error.txt', 'a+') as f:
        #         f.write(directory + '\n')
